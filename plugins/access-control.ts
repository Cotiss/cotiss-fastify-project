...
export type AccessPreHandlerFactory = (
  permissions: Permissions[]
) => preHandlerHookHandler;

const accessControlPlugin: FastifyPluginAsync = async function (instance) {
  const requireAllPermissions: AccessPreHandlerFactory = function (
  ...
  const requireOnePermissions: AccessPreHandlerFactory = function (
    requiredPermissions: Permissions[] = []
  ) {
    return function (request, reply) {
      try {
